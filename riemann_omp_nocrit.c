...
int main(int argc, char *argv[]) {
    ...
    // Reserva memoria para almacenar los resultados locales de cada thread
    double *resultados_locales = (double *)malloc(num_threads * sizeof(double));
    ...
    #pragma omp parallel
    {
        ...
        // Calcula la suma de Riemann para el intervalo local y almacena el resultado en un arreglo
        resultados_locales[thread_id] = trapezoides(a_local, b_local, n_local);
    }

    // Acumula los resultados parciales de todos los threads
    double resultado = 0.0;
    for (int i = 0; i < num_threads; i++) {
        resultado += resultados_locales[i];
    }
    ...
    // Libera la memoria reservada para los resultados locales
